...
async function loadAI() {
    ...
    try {
        ...
        set("trendCard", data.trend || "--");

        set("stopLoss", data.stopLoss || "--");
        set("tradeSL", data.stopLoss || "--");

        set("tp1", data.tp1 || data.takeProfit || "--");
        set("tp2", data.tp2 || "--");
        set("tp3", data.tp3 || "--");

        set("tradeTP1", data.tp1 || data.takeProfit || "--");
        set("tradeTP2", data.tp2 || "--");
        set("tradeTP3", data.tp3 || "--");

        set("tradeSignal", data.recommendation || "--");
        set("tradeConfidence", data.confidence || "--");
        set("tradeBias", data.marketBias || "--");
        ...
        set("summaryRisk", data.riskLevel || "--");
...
async function loadSentiment() {
    ...
    try {
        ...
        set("sentimentCard", data.sentiment || "--");
...
async function loadZones() {
    ...
    try {
        ...
        if (!res.ok) throw new Error("Zones API Error");
...
async function loadAnalysis() {
    ...
    try {
        ...
        setHTML(
            "marketReason",
            data.marketReason || "Waiting for AI reasoning..."
        );
